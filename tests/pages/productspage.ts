...
export class ProductsPage {
  ...
  async getAllTheProducts() {
    ...
    for (let productsCard of productsCardLocators) {
      let product: Product = {
        ...
        price: returnNumbersFromText(
          await productsCard.locator("xpath=/p[2]").innerText(),
        ),
  ...
  getLowestOrHighestPricedProduct(
    products: Product[],
    priceType: string = "lowest",
  ) {
    if (priceType === "lowest") {
      return products.reduce(
        (minPriceProduct: Product, currentProduct: Product) => {
          return Number(currentProduct.price) < Number(minPriceProduct.price)
            ? currentProduct
            : minPriceProduct;
        },
      );
      ...
      return products.reduce(
        (maxPriceProduct: Product, currentProduct: Product) => {
          return Number(currentProduct.price) > Number(maxPriceProduct.price)
            ? currentProduct
            : maxPriceProduct;
        },
      );
  ...
  async verifyNumberOfProductsShowedInCartButton(products: Product[]) {
    let cartProductCount = returnNumbersFromText(
      await this.cartButton.innerText(),
    );
  ...
  async addProductsToCart(productsToOrder: Product[]) {
    ...
    for (let product of productsToOrder) {
      let productsFilteredBasedOnName = this.filterProductsBasedOnName(
        products,
        product["nameContains"],
      );
      let productFilterBasedOnPriceType = this.getLowestOrHighestPricedProduct(
        productsFilteredBasedOnName,
        product["priceType"],
      );
