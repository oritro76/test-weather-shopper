...
export class CartPage{
    ...
    constructor(page: Page) {
        ...
        this.payWithCardButton =  this.page.getByRole("button").filter({ hasText: 'Pay' });
    ...
    async verifyAddedToCartProductsAndPricesAreShown(addedToCartProducts:Product[], checkoutPageOrderedProducts:Product[]){
        
        expect(addedToCartProducts).toMatchObject(checkoutPageOrderedProducts);
    }

    async verifyTotalPrice(addedToCartProducts:Product[]){
        let totalCheckoutPrice = returnNumbersFromText(await this.totalCheckoutPrice.innerText());
    ...
    async payWithStripe(email:string, 
                        cardNumber:string, 
                        cardExpiry:string, 
                        cardCVV:string, 
                        zipCode:string){
