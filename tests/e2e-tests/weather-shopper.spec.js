...
test('pay with stripe', async ({ page }) => {
    ...
    const productsAddedToCart = await productsPage.addProductsToCart(productsToAddToCart);
    console.log(productsAddedToCart);
    await productsPage.verifyNumberOfProductsShowedInCartButton(productsAddedToCart);
    await productsPage.clickOnCartButton();
    const checkoutPageOrderedProducts = await cartPage.getOrderedItems();
    await cartPage.verifyAddedToCartProductsAndPricesAreShown(productsAddedToCart, checkoutPageOrderedProducts);
    await cartPage.verifyTotalPrice(productsAddedToCart);
    await cartPage.payWithStripe("test@test.com",
                                  "4000056655665556",
                                  "1225",
                                  "123",
                                  "123"
